...
class Node:
  def __init__(self, iD=None,state=None,expansionSequence=None,children=None, parent=None,actions=None,removed=False,cost=0):
    self.state = state
    self.parent = parent
    self.children = []
    self.iD = iD
...
class Player():
    ...
    def run(self, problem):
        ...
        if self.regenerate==True:
            ...
            self.frontier.append(Node(self.IDnum,starting_point,None,None,None,None,False,100))
            ...
            while found!=True:
                ...
                for x in range(len(self.frontier)):
                    if self.frontier[x].cost<smallestCost:
                        wantedIndex = x
                        smallestCost=self.frontier[x].cost
                ...
                childIDList=[]
                maze_size=self.size
                row=self.frontier[wantedIndex].state[1]
                coll=self.frontier[wantedIndex].state[0]
                ...
                if coll!=0:
                    ...
                    children.append(Node(self.IDnum,[coll-1,row],-1,None,self.frontier[wantedIndex],None,False,abs(row-goal[1])+ abs(coll-1-goal[0])))
                ...
                if row!=0:
                    ...
                    dirList.append('n')
                    childIDList.append(self.IDnum)
                    children.append(Node(self.IDnum,[coll,row-1],-1,None,self.frontier[wantedIndex],None,False,abs(row-1-goal[1])+ abs(coll-goal[0])))
                if row!=maze_size[0]-1:
                    ...
                    childIDList.append(self.IDnum)
                    children.append(Node(self.IDnum,[coll,row+1],-1,None,self.frontier[wantedIndex],None,False,abs(row+1-goal[1])+ abs(coll-goal[0])))
                self.frontier[wantedIndex].actions=dirList
                self.frontier[wantedIndex].children=childIDList
                self.ExpSeq=self.ExpSeq+1
                self.frontier[wantedIndex].expansionSequence=self.ExpSeq
                
                self.explored.append(self.frontier[wantedIndex])
                self.exploredWChildren.append(self.frontier[wantedIndex])
                del self.frontier[wantedIndex]
                for child in children:
                    
                    if (child.state in [front.state for front in self.frontier]) or (child.state in [expl.state for expl in self.explored]):
                        child.removed=True
                    self.exploredWChildren.append(child)
                    if not(child.state in [front.state for front in self.frontier]) and not (child.state in [expl.state for expl in self.explored]):
                        if child.state==goal:
            ...
            for expl in range(len(self.exploredWChildren)):
                if self.exploredWChildren[expl].parent==None:
                    ...
                    {"id":self.exploredWChildren[expl].iD,
                     ...
                     "parent":None
                     # "cost":self.exploredWChildren[expl].cost
                        })
                else:
                    self.searchTreeList.insert(len(self.searchTreeList),
                                       {"id":self.exploredWChildren[expl].iD,
