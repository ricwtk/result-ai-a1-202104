...
class Player():
	...
	def setHeuristics(self, sz, src):
		h = [[0 for i in range(sz[0])] for j in range(sz[1])]
		...
		while(len(queue) > 0):
			...
			for child in childs:
				if((child[0] >= 0) and (child[0] < sz[0]) and (child[1] >= 0) and (child[1] < sz[1]) and (visited[child[0]][child[1]]) == False):
					queue.append(child)
					visited[child[0]][child[1]] = True
					h[child[0]][child[1]] = h[node[0]][node[1]] + 1
	...
	def run(self, problem):
		...
		search_tree = []
		solution = []
		h = self.setHeuristics(sz, destination)
		direction = "nsew"
		...
		hq = [(h[src[0]][src[1]], currId)]
		heapq.heapify(hq)

		while True:
			idx = heapq.heappop(hq)[1]
			...
			for i in range(len(childs)):
				if(childs[i][0] >= 0 and childs[i][0] < sz[0] and childs[i][1] >= 0 and childs[i][1] < sz[1]):
					...
					if(visited[childs[i][0]][childs[i][1]] == True):
						...
						heapq.heappush(hq, (h[childs[i][0]][childs[i][1]], currId))
