class Player():
    name = "uninformed_BreadthFirstSearch"
    informed = False
    group = "Paiseh"
    members = [
    ...
    def run(self, problem): 
        ...
        frontier.append({"parent": None, "snake": snake, "id": count})
        ...
        while not found_goal and len(frontier) != 0: 
            ...
            for action in node['actions']:
                ...
                if new_snake[0] not in explored and new_snake[0] not in [f['snake'][0] for f in frontier]:
                    ...
                    # Goal test when node is generated
                    for f in food:
                        if new_snake[0] == f:
                            found_goal = True
                            goal = child_node

                    frontier.append({"parent": node['id'], "snake": new_snake, "id": count})
                else:
                    child_node = {
                        "id": count,
                        "state": ",".join([str(num_str) for num_str in new_snake[0]]),
