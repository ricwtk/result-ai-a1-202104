...
class Player():
  # Class variables.
  name = "Greedy Best-First Search (GBFS) Algorithm"
  group = "Big bois"
  members = [
    ["Tan Hanley", "18033936"],
  ...
  # Is used as a key for sorting through the frontier list.
  def takeDistance(self, arr):
    return arr[-1]
  ...
  def gbfs(self, initial_state, goal_state):
    ...
    while not found_goal:
      ...
      # Create a copy of the frontier to be sorted.
      temp = copy.deepcopy(frontier)
      # Append the distance value of a node from the goal node to the list.
      for t in temp:
        distance = 0
        distance = abs(goal_state[0] - t[-1][0]) + abs(goal_state[1] - t[-1][1])
        t.append(distance)
      
      # Sort the frontier based on distance from the goal node.
      temp.sort(key = self.takeDistance)
      # Remove the distance values of the nodes from the goal node from the list.
      for i in temp:
        del i[-1]
      
      # Convert the old frontier into the new sorted frontier.
      frontier = temp
  ...
  def run(self, problem):
    ...
    addDirection = [ ['n', [0, -1]], ['s', [0, 1]], ['w', [-1, 0]], ['e', [1, 0]] ]
    directions = "nswe"
    coordinates = self.gbfs(initial_state, goal_state)
